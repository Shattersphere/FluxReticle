...
- Ship-system cooldown marker anchored at a fixed screen-relative offset from the flux cursor head, with configurable fade, position, ring shape, alpha, and RGBA state colors.
...
- The ship-system marker stays at the configured fixed offset from the flux cursor head when the cursor points above, below, left, or right of the ship.
